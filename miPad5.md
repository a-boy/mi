...
html <div id="app">   <h1>{{ message }}</h1>   <p v-if="show">This is visible!</p>   <ul>     <li v-for="item in items" :key="item.id">{{ item.text }}</li>   </ul>   <button @click="changeMessage">Change Message</button> </div>  <script>   const { createApp } = Vue    createApp({     data() {       return {         message: 'Hello Vue 3!',         show: true,         items: [           { id: 1, text: 'Item 1' },           { id: 2, text: 'Item 2' }         ]       }     },     methods: {       changeMessage() {         this.message = 'Message Changed!'       }     }   }).mount('#app') </script> 
...
html <div id="app">   <h1>{{ count }}</h1>   <button @click="increment">Increment</button> </div>  <script>   import { createApp, ref } from 'vue'    createApp({     setup() {       const count = ref(0)        const increment = () => {         count.value++       }        return {         count,         increment       }     }   }).mount('#app') </script> 
